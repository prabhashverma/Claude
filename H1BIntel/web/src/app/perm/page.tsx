...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, string> = {
    ...
    certified_expired: "bg-orange-100 text-orange-700",
  };
  return (
...
export default function PERMPage() {
  ...
  const [results, setResults] = useState<PERMFiling[]>([]);
  ...
  const doSearch = useCallback(async (p = 1) => {
    ...
    const res = await fetch(`/api/perm?${params}`);
    const data = await res.json();
    setResults(data.results);
    setTotal(data.total);
    setPage(p);
    setLoading(false);
  }, [employer, title, state, citizenship, status, sortBy, sortDir]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-2xl font-bold text-gray-900 mb-1">PERM Search</h1>
      <p className="text-sm text-gray-500 mb-6">
        Search green card (PERM) labor certification filings. FY2025 data.
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-4">
        ...
        <input
          type="text"
          placeholder="Citizenship (e.g. India)..."
          value={citizenship}
        ...
        >
          ...
          <option value="certified_expired">Certified-Expired</option>
        </select>
      </div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        >
          ...
          <option value="processing_days:desc">Processing: Longest</option>
          <option value="processing_days:asc">Processing: Fastest</option>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 text-left text-gray-500 text-xs uppercase">
              ...
              <th className="pb-2 pr-3 font-medium text-right">Prevailing</th>
              <th className="pb-2 pr-3 font-medium">Citizenship</th>
            ...
            {results.map((r) => (
              <tr key={r.case_number} className="border-b border-gray-50 hover:bg-gray-50">
                ...
                <td className="py-2.5 pr-3 text-right tabular-nums text-gray-500">
                  {formatWage(r.prevailing_wage)}
                </td>
                <td className="py-2.5 pr-3 text-gray-600 text-xs">
                  {r.worker_citizenship || "—"}
                </td>
                <td className="py-2.5 pr-3 text-right tabular-nums text-gray-500">
                  {r.processing_days != null ? r.processing_days : "—"}
                </td>
                <td className="py-2.5">
